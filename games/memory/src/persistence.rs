//! Memory persistence — best times per difficulty.
...
/// Typed handle to Memory's best-times file.
pub type BestTimes = Persistence<HashMap<String, f32>>;

/// Build the default handle: `best_times.ron` next to Memory's manifest.
...
mod tests {
    ...
    fn test_update_if_better_new_key_inserts() {
        ...
        assert!(update_if_better(&mut m, "Easy", 10.0));
    ...
    fn test_update_if_better_slower_is_rejected() {
        ...
        assert!(!update_if_better(&mut m, "Easy", 15.0));
        assert_eq!(m.get("Easy"), Some(&10.0));
    }
    ...
    fn test_update_if_better_faster_replaces() {
        let mut m = HashMap::new();
        m.insert("Easy".to_string(), 10.0);
        assert!(update_if_better(&mut m, "Easy", 8.0));
