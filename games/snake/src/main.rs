...
fn draw_walls(walls: &[snake::components::Wall], ctx: &systems::GameContext) {
    ...
    for w in walls {
        ...
        draw_rectangle(r.x + pad, r.y + pad, r.w - 2.0 * pad, r.h - 2.0 * pad, w.color);
    }
}
...
fn draw_snake(snake: &snake::components::Snake, ctx: &systems::GameContext) {
    ...
    for (i, cell) in snake.body.iter().enumerate() {
        ...
        let color = if i == 0 { snake.color_head } else { snake.color_body };
        draw_rectangle(r.x + pad, r.y + pad, r.w - 2.0 * pad, r.h - 2.0 * pad, color);
    }
}
...
fn draw_food(food: &snake::components::Food, ctx: &systems::GameContext) {
    ...
    let pad = 4.0;
    draw_rectangle(r.x + pad, r.y + pad, r.w - 2.0 * pad, r.h - 2.0 * pad, food.color);
}
...
async fn main() {
    ...
    loop {
        ...
        draw_text(format!("Best: {}", best), screen_w - 130.0, 24.0, 28.0, WHITE);
        let speed_level = world.snake_state.food_eaten / ctx.food_per_speedup;
        draw_text(format!("Speed: {}", speed_level + 1), screen_w - 130.0, 52.0, 24.0, GRAY);
        draw_text(
            format!(
                "Level {}/{}  -  {}/{}",
                current_level + 1, MAX_LEVELS,
                world.snake_state.food_eaten, world.snake_state.target_score,
            ),
            screen_w / 2.0 - 120.0, 24.0, 24.0, WHITE,
        );
        ...
        match world.state {
            GameState::Start => {
                ...
                draw_centered("Press SPACE to start", screen_w / 2.0, screen_h / 2.0 + 20.0, 30, WHITE);
                draw_centered("Arrows / WASD to move", screen_w / 2.0, screen_h / 2.0 + 60.0, 24, GRAY);
                draw_centered(
                    &format!("Level {} - {} walls", current_level + 1, world.walls.len()),
                    screen_w / 2.0, screen_h / 2.0 + 100.0, 20, GRAY,
                );
            }

            GameState::LevelCleared => {
                draw_centered("LEVEL CLEARED!", screen_w / 2.0, screen_h / 2.0, 60, GREEN);
                draw_centered("Next level...", screen_w / 2.0, screen_h / 2.0 + 50.0, 30, WHITE);
            }
            ...
            GameState::GameOver => {
                ...
                draw_centered(
                    ...
                    screen_w / 2.0, screen_h / 2.0 + 50.0, 32, WHITE,
                );
                draw_centered(
                    "Press R to restart",
                    screen_w / 2.0, screen_h / 2.0 + 100.0, 30, WHITE,
                );
            }
            ...
            GameState::Win => {
                ...
                draw_centered(
                    ...
                    screen_w / 2.0, screen_h / 2.0 + 60.0, 40, WHITE,
                );
                draw_centered(
                    "Press R to restart",
                    screen_w / 2.0, screen_h / 2.0 + 120.0, 30, WHITE,
                );
            }
